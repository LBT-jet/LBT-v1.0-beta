...
//...........................................tau-eta coordinates		  
		  ...
		  if(pc0[0]<sqrt(qhat00))
		  {
		  free=1;
		  }
		  
		  if(pc0[0]<Ecut*temp00)
		  {
		  free=1;
          }
...
//...........test			 
			  ...
			  if(P[0][idlead]>P[0][i])   //i<=nj
                {	
                  double KATT_temp = KATT1[i];
				  KATT1[i]=KATT1[idlead];
		          KATT1[idlead]=KATT_temp;					

                  double P_temp[4] = {0};
                  double V_temp[4] = {0};
                  double Vfrozen_temp[4] = {0};
				  for(int j=0;j<=3;j++)
					{
					  P_temp[j]=P[j][i];
					  P[j][i]=P[j][idlead];
					  ...
					  Vfrozen_temp[j]=Vfrozen[j][i];
					  Vfrozen[j][i]=Vfrozen[j][idlead];
					  Vfrozen[j][idlead]=Vfrozen_temp[j];
					}

                  double tiform_temp = tiform[i];
                  tiform[i] = tiform[idlead];
                  tiform[idlead] = tiform_temp; 
                }
...
temp0=0.0;                  
temp00=0.0; 
Ecut=0.0;                     //energy cut 6*temp of the partons
